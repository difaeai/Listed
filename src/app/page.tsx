...
function HomePagePlanSelector() {
    ...
    const planDetailsData: { [key in PlanKey]: Omit<PlanDetails, 'features' | 'className' | 'priceKey'> & { priceKey: PlanKey } } = {
    ...
    const isProceedEnabled = businessType !== null && (
        (businessType === 'offline' && selectedCountry !== null && selectedPlan !== null && selectedMonths > 0) ||
        (businessType === 'online' && selectedCategory !== null && selectedPlan !== null && selectedMonths > 0)
    );
    
    const calculateTotalPrice = () => {
        if (!selectedPlan || !pricing) return { main: 0, pkr: 0 };
        const pricePerMonth = pricing[selectedPlan];
        const mainTotal = pricePerMonth * selectedMonths * (selectedMonths === 12 ? 0.8 : 1);
        const pkrTotal = mainTotal * pricing.dollarRate;
        return { main: mainTotal, pkr: pkrTotal };
    };
    ...
    const OfflineFlow = () => (
        <>
            ...
            <div className="space-y-1.5">
                ...
                <Select onValueChange={(v) => setSelectedMonths(parseInt(v, 10))} disabled={!selectedPlan} value={selectedMonths > 0 ? selectedMonths.toString() : ""}>
                  <SelectTrigger id="duration-select-main"><SelectValue placeholder={selectedPlan ? "Select duration..." : "Please choose an investor type first"} /></SelectTrigger>
                  <SelectContent>{Array.from({ length: 12 }, (_, i) => i + 1).map(m => (<SelectItem key={m} value={m.toString()}>{m} Month{m > 1 ? 's' : ''} {m === 12 && '(20% OFF)'}</SelectItem>))}</SelectContent>
                </Select>
            </div>
        </>
    );
    ...
    const OnlineFlow = () => (
        <>
            ...
            <div className="space-y-1.5">
                <Label htmlFor="duration-select-online">4. Choose Duration</Label>
                <Select onValueChange={(v) => setSelectedMonths(parseInt(v, 10))} disabled={!selectedPlan} value={selectedMonths > 0 ? selectedMonths.toString() : ""}>
                  <SelectTrigger id="duration-select-online"><SelectValue placeholder={selectedPlan ? "Select duration..." : "Please choose an investor type first"} /></SelectTrigger>
                  <SelectContent>{Array.from({ length: 12 }, (_, i) => i + 1).map(m => (<SelectItem key={m} value={m.toString()}>{m} Month{m > 1 ? 's' : ''} {m === 12 && '(20% OFF)'}</SelectItem>))}</SelectContent>
                </Select>
            </div>
        </>
