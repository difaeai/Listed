...
export default function InvestorComplaintsPage() {
  ...
  useEffect(() => {
    if (authLoading || !authUser || authUser.type !== 'investor' || !db) {
  ...
  const onSubmitComplaint = async (values: ComplaintFormValues) => {
    ...
    const complaintBase: Omit<Complaint, 'id' | 'createdAt' | 'updatedAt' | 'status' | 'adminNotes' | 'targetUserIdentifier'> & { createdAt: any, updatedAt: any, status: ComplaintStatus } = {
      ...
      complainantName: authUser.name || "Investor",
      complainantEmail: authUser.email,
      complainantType: 'investor',
  ...
  const getStatusBadgeVariant = (status: ComplaintStatus): "default" | "secondary" | "outline" | "destructive" => {
    switch (status) {
      ...
      case 'Resolved': return "default"; // default is primary color
  ...
  if (authUser?.type !== 'investor') {
    return <div className="container mx-auto py-8 px-4 md:px-6 text-center">Access Denied. This page is for Investors.</div>;
  }
