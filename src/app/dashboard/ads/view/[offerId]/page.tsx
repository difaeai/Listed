...
import { ArrowLeft, Eye, Users, DollarSign, Briefcase, FileText, Mail, CalendarDays, MessageSquare, ThumbsUp, ThumbsDown, Percent, Edit, Package, Zap as ZapIcon, ImageIcon, Info as InfoIcon } from 'lucide-react';
...
import { doc, getDoc, collection, query, where, onSnapshot, Timestamp, increment, setDoc, updateDoc, serverTimestamp } from "firebase/firestore";
import NextImage from 'next/image';

// For Sales Professional Liker display
interface SalesProfessionalLiker {
  id: string; 
  userName: string; 
  userAvatarSeed?: string;
}

export default function CorporationViewOfferDetailPage() {
  ...
  const [interestedProfessionals, setInterestedProfessionals] = useState<SalesProfessionalLiker[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubOffer = onSnapshot(offerDocRef, (docSnap) => {
        if (docSnap.exists()) {
            ...
            if (offerData.corporationId !== authUser.uid) {
                toast({ title: "Unauthorized", description: "You can only view details of your own offers here.", variant: "destructive" });
                router.push("/dashboard/ads");
                setOffer(null);
            } else if (offerData.isDeletedByAdmin) {
                 toast({ title: "Offer Removed", description: "This offer has been removed by an administrator.", variant: "destructive"});
                 router.push("/dashboard/ads");
                 setOffer(null);
            }
             else {
                setOffer(offerData);
                setSalesProFeedback({
                    positive: offerData.positiveResponseRate || 0,
        ...
        console.error("Error fetching offer details for corporation:", error);
    ...
    const feedbackColRef = collection(db, "platformOffers", offerId, "feedback");
    const qFeedback = query(feedbackColRef, where("type", "==", "positive")); 
    const unsubFeedback = onSnapshot(qFeedback, async (feedbackSnapshot) => {
        const professionals: SalesProfessionalLiker[] = [];
        for (const feedbackDoc of feedbackSnapshot.docs) {
            ...
            if (feedbackData.userId) {
                ...
                if (userSnap.exists() && userSnap.data()?.type === 'professional') {
                    const userData = userSnap.data();
                    professionals.push({
                        id: userSnap.id,
                        userName: userData.name || "Sales Pro",
        ...
        console.error("Error fetching interested professionals:", error);
    });
  ...
  if (!offer) {
    return (
      <div className="container mx-auto py-8 px-4 md:px-6 text-center">
        <h2 className="text-2xl font-semibold mb-4">Offer Not Found or Access Denied</h2>
        <Button asChild><Link href="/dashboard/ads"><ArrowLeft className="mr-2 h-4 w-4" /> Back to Manage Offers</Link></Button>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8 px-4 md:px-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button variant="outline" asChild>
            <Link href={`/dashboard/ads/${offer.id}/edit`}><Edit className="mr-2 h-4 w-4"/>Edit Offer</Link>
        </Button>
      </div>
      ...
      <Card className="shadow-xl rounded-xl overflow-hidden">
        <CardHeader className="bg-muted/30 p-6">
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            ...
            <div className="text-sm text-muted-foreground text-left md:text-right">
              ...
              <p className="flex items-center gap-1.5">{getOfferCategoryIcon(offer.offerCategory)}Category: {offer.offerCategory}</p>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-6">
          {offer.mediaUrl && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold mb-2 flex items-center">
               ...
               <NextImage src={offer.mediaUrl} alt={offer.title} width={800} height={450} className="rounded-md object-contain mx-auto border" data-ai-hint="offer image large"/>
            </div>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <Dialog>
             ...
             <Dialog>
                <DialogTrigger asChild>
                    ...
                    <CardHeader className="pb-2"><CardTitle className="text-base flex items-center"><ThumbsUp className="mr-2 h-5 w-5 text-green-700"/>Interested Pros</CardTitle></CardHeader>
                    <CardContent><p className="text-3xl font-bold">{interestedProfessionals.length}</p><p className="text-xs text-muted-foreground">Sales pros who showed positive interest.</p></CardContent>
                    </Card>
                </DialogTrigger>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle>Professionals Interested in "{offer.title}"</DialogTitle>
            ...
            <Card className="shadow-sm bg-red-500/10 border-red-500/30">
              ...
              <CardContent><p className="text-3xl font-bold">{salesProFeedback.negative}</p><p className="text-xs text-muted-foreground">Sales pros who gave negative feedback.</p></CardContent>
            </Card>
          </div>

          <div className="space-y-6">
            <div><h3 className="text-lg font-semibold mb-1 flex items-center"><InfoIcon className="mr-2 h-5 w-5 text-primary"/>Offer Description</h3><p className="text-muted-foreground whitespace-pre-line text-sm leading-relaxed">{offer.description}</p></div>
            <Separator/>
            <div><h3 className="text-lg font-semibold mb-2">Key Details</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div className="flex items-center gap-2 p-3 bg-muted/50 rounded-md"><DollarSign className="h-5 w-5 text-primary"/><div><span className="text-muted-foreground">Commission:</span><p className="font-semibold">{offer.commissionRate} ({offer.commissionType})</p></div></div>
        ...
        <CardFooter className="p-6 border-t bg-muted/30"><p className="text-xs text-muted-foreground">This is the detailed view of your offer as it appears to your sales network. You can edit it or monitor its engagement here.</p></CardFooter>
      </Card>
    </div>
  );
}
