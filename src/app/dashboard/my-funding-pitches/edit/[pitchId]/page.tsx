...
const pitchFormSchema = z.object({
  ...
  industry: z.enum(industries as [IndustryType, ...IndustryType[]], { required_error: "Please select an industry." }),
            ...
            <CardContent className="space-y-6">
              ...
              <FormField control={form.control} name="pitchImageFile" render={({ field }) => (
                <FormItem><FormLabel>New Pitch Image (Optional, Up to ${MAX_FILE_SIZE_MB}MB)</FormLabel>
                  {currentImageUrl && <div className="mb-2"><img src={currentImageUrl} alt="Current pitch" className="rounded-md max-w-xs max-h-40 object-contain border p-1" /></div>}
                  <FormControl><Input type="file" accept="image/*" ref={pitchImageFileRef} onChange={(e) => field.onChange(e.target.files)} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}/>
