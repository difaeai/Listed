...
export default function CreateCorporationPitchPage() {
  ...
  const currentProjectSummary = form.watch("projectSummary");
  ...
  const handleUseAiSummary = () => {
    if (!aiImprovedSummary) return;
    const combinedSummary = `## ${aiImprovedSummary.suggestedTitle}\n\n**Hook:** ${aiImprovedSummary.hook}\n\n**Problem:** ${aiImprovedSummary.problem}\n\n**Solution:** ${aiImprovedSummary.solution}\n\n**Unique Value:** ${aiImprovedSummary.uniqueValue}\n\n**Call to Action:** ${aiImprovedSummary.callToAction}`;
    form.setValue("projectSummary", combinedSummary, { shouldValidate: true, shouldDirty: true });
    setIsAiDialogOpenn(false);
  };
  ...
  async function onSubmit(data: PitchFormValues) {
    ...
    if (data.pitchImageFile && data.pitchImageFile.length > 0) {
      ...
      try {
        ...
        toast({ title: "Image Processed", description: `Image "${file.name}" is ready.`});
      } catch (error) {
    ...
    try {
      const docRef = await addDoc(collection(db, "fundingPitches"), {
      ...
      toast({
        ...
        description: `Your pitch "${data.projectTitle}" has been posted.`,
        variant: "default",
      });
      router.push("/dashboard/my-funding-pitches");
    } catch (error: any) {
      console.error("Failed to save pitch to Firestore:", error);
      if (error.message && error.message.includes("exceeds the maximum allowed size")) {
        toast({
          title: "Error: Document Too Large",
          description: `The pitch data (possibly due to a large image Data URI) exceeds Firestore's 1MiB limit. Please use a smaller image (under ${MAX_FILE_SIZE_MB}MB) or shorten text content.`,
          variant: "destructive",
          duration: 12000,
        });
      } else {
        toast({
          ...
          description: `Could not save your pitch: ${error.message}`,
          variant: "destructive",
        });
      }
    } finally {
