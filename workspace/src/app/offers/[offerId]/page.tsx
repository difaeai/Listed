...
export default function OfferDetailPage() {
  ...
  const handleFeedback = async (type: 'positive' | 'negative') => {
    if (!offer || !authUser || !authUser.uid || hasGivenFeedback || !db) {
            ...
            <CardContent className="p-6 space-y-6">
                ...
                <div id="interaction-section" className="space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <Button onClick={() => handleFeedback('positive')} variant="outline" className="flex-1 border-green-500 text-green-600 hover:bg-green-50 hover:text-green-700 disabled:opacity-70 disabled:cursor-not-allowed" disabled={hasGivenFeedback || authUser?.uid === offer.creatorId}>
                            <ThumbsUp className="mr-2 h-5 w-5" /> Positive Feedback ({offer.positiveResponseRate || 0})
                        </Button>
                        <Button onClick={() => handleFeedback('negative')} variant="outline" className="flex-1 border-red-500 text-red-600 hover:bg-red-50 hover:text-red-700 disabled:opacity-70 disabled:cursor-not-allowed" disabled={hasGivenFeedback || authUser?.uid === offer.creatorId}>
                ...
                <div id="message-section" className="space-y-4">
                    ...
                    <Textarea
                        ...
                        disabled={authUser?.uid === offer.creatorId}
                    />
                     <div className="flex flex-col sm:flex-row gap-3">
                        <Button onClick={handleSendMessage} disabled={!message.trim() || authUser?.uid === offer.creatorId} className="w-full sm:w-auto flex-1">
                         ...
                         <Button 
                            ...
                            disabled={isSubscriptionProcessing || (authUser ? offer.creatorId === authUser.uid : true)}
                        >
