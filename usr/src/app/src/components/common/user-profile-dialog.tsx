...
export function UserProfileDialog({ userId, isOpen, onOpenChange, currentLoggedInUser }: UserProfileDialogProps) {
  ...
  const [isMessageSubDialogOpen, setIsMessageSubDialogOpen] = useState(false);
  ...
  const handleOpenMessageSubDialog = () => {
      ...
      setMessageBody("");
      setIsMessageSubDialogOpen(true);
  };
  ...
  const handleSendMessageToProfileUser = async () => {
      ...
      else if(currentLoggedInUser.type === 'corporation' && userProfile.type === 'professional') messageType = 'corporation_to_salespro';
      else if(currentLoggedInUser.type === 'professional' && userProfile.type === 'investor') messageType = 'salespro_to_investor';
      else if(currentLoggedInUser.type === 'professional' && userProfile.type === 'corporation') messageType = 'salespro_to_corporation';
      ...
      else if(currentLoggedInUser.type === 'investor' && userProfile.type === 'corporation') messageType = 'investor_to_corporation';
      else if(currentLoggedInUser.type === 'corporation' && userProfile.type === 'investor') messageType = 'corporation_to_investor';
      ...
      try {
          ...
          setIsMessageSubDialogOpen(false);
      } catch (error) {
  ...
  const isActiveMonthlySub = userProfile?.subscriptionType === 'monthly' && userProfile?.subscriptionExpiryDate && new Date(userProfile.subscriptionExpiryDate) > new Date();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
        ...
        {isLoading ? (
          ...
          {currentLoggedInUser && currentLoggedInUser.uid !== userProfile.uid && (
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Button onClick={handleOpenMessageSubDialog} className="flex-1 bg-primary hover:bg-primary/90" disabled={isMessagingBlockedOverall}>
      ...
      <Dialog open={isMessageSubDialogOpen} onOpenChange={setIsMessageSubDialogOpen}>
        <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
            <Button variant="outline" onClick={() => setIsMessageSubDialogOpen(false)}>Cancel</Button>
