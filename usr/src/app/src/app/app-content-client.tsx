...
export function AppContentClient({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      // If user is an admin and in the admin area, do nothing. Let the admin layout handle it.
      ...
      if (currentUser.type === 'admin') {
        ...
        userDashboardPath = isPaymentVerificationNeeded ? "/offers/verify-payment" : "/offers";
        basePath = '/offers';

        // **NEW LOGIC**: Enforce redirection for expired users
        if (isPaymentVerificationNeeded && pathname !== '/offers/verify-payment') {
            router.replace('/offers/verify-payment?reason=subscription_expired');
            return;
        }
      }
      ...
      const isCorrectPage = pathname.startsWith(basePath);
      const isVerificationPage = pathname === '/offers/verify-payment';
      
      if (!isCorrectPage && !(userDashboardPath === '/offers/verify-payment' && isVerificationPage)) {
         if (currentUser.type !== 'admin' || !isAdminArea) {
            router.replace(userDashboardPath);
            return;
         }
